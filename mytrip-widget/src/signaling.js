                            ...
                            this.accepted(event.jsep)
                                .then(() => {
                                    ...
                                    if (this.outgoingDuration > callInfinity) {
                                        const timeout = this.getRandomMillisecond(this.outgoingDuration, this.outgoingDeviation);
                                        console.info(`[${this.callId}] Set outgoing call timeout to: ${timeout}`);
    ...
    async initPeerConnection() {
        return new PeerConnection({
            iceCandidateCallback: (candidate) => {
                this.client?.execute('ice_trickle', {
                    line: this.line,
                    candidate,
                }).then(() => {
